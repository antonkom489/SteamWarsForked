#include "SWAT_PlayMontageForMeshAndWaitForEvent.h"

#include "AbilitySystemGlobals.h"
#include "Characters/CharacterComponents/AbilitySystem/SWAbilitySystemComponent.h"
#include "GameFramework/Character.h"

USWAT_PlayMontageForMeshAndWaitForEvent::USWAT_PlayMontageForMeshAndWaitForEvent(const FObjectInitializer& ObjectInitializer)
...
USWAbilitySystemComponent* USWAT_PlayMontageForMeshAndWaitForEvent::GetTargetASC()
{
	return Cast<USWAbilitySystemComponent>(AbilitySystemComponent);
}

void USWAT_PlayMontageForMeshAndWaitForEvent::OnMontageBlendingOut(UAnimMontage* Montage, bool bInterrupted)
{
...
void USWAT_PlayMontageForMeshAndWaitForEvent::OnAbilityCancelled()
{
...
void USWAT_PlayMontageForMeshAndWaitForEvent::OnMontageEnded(UAnimMontage* Montage, bool bInterrupted)
{
	if (!bInterrupted)
	{
		if (ShouldBroadcastAbilityTaskDelegates())
...
void USWAT_PlayMontageForMeshAndWaitForEvent::OnGameplayEvent(FGameplayTag EventTag, const FGameplayEventData* Payload)
{
	if (ShouldBroadcastAbilityTaskDelegates())
	{
...
USWAT_PlayMontageForMeshAndWaitForEvent* USWAT_PlayMontageForMeshAndWaitForEvent::PlayMontageForMeshAndWaitForEvent(UGameplayAbility* OwningAbility,
	...
	float OverrideBlendOutTimeForStopWhenEndAbility)
{
	...
	USWAT_PlayMontageForMeshAndWaitForEvent* MyObj = NewAbilityTask<USWAT_PlayMontageForMeshAndWaitForEvent>(OwningAbility, TaskInstanceName);
	MyObj->Mesh = InMesh;
...
void USWAT_PlayMontageForMeshAndWaitForEvent::Activate()
{
	if (Ability == nullptr)
	{
		return;
	...
	USWAbilitySystemComponent* GSAbilitySystemComponent = GetTargetASC();
	...
	if (GSAbilitySystemComponent)
	{
		...
		if (AnimInstance != nullptr)
		{
			...
			/*if (GSAbilitySystemComponent->PlayMontageForMesh(Ability, Mesh, Ability->GetCurrentActivationInfo(), MontageToPlay, Rate, StartSection, bReplicateMontage) > 0.f)
			{
				...
				CancelledHandle = Ability->OnGameplayAbilityCancelled.AddUObject(this, &USWAT_PlayMontageForMeshAndWaitForEvent::OnAbilityCancelled);

				BlendingOutDelegate.BindUObject(this, &USWAT_PlayMontageForMeshAndWaitForEvent::OnMontageBlendingOut);
				AnimInstance->Montage_SetBlendingOutDelegate(BlendingOutDelegate, MontageToPlay);

				MontageEndedDelegate.BindUObject(this, &USWAT_PlayMontageForMeshAndWaitForEvent::OnMontageEnded);
...
void USWAT_PlayMontageForMeshAndWaitForEvent::ExternalCancel()
{
...
void USWAT_PlayMontageForMeshAndWaitForEvent::OnDestroy(bool AbilityEnded)
{
...
bool USWAT_PlayMontageForMeshAndWaitForEvent::StopPlayingMontage(float OverrideBlendOutTime)
{
	if (Mesh == nullptr)
	{
		return false;
...
FString USWAT_PlayMontageForMeshAndWaitForEvent::GetDebugString() const
{
