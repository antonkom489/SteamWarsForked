#include "SWGATA_Trace.h"

#include "AbilitySystemComponent.h"

ASWGATA_Trace::ASWGATA_Trace()
{
...
void ASWGATA_Trace::ResetSpread()
{
	bUseAimingSpreadMod = false;
	BaseSpread = 0.0f;
	AimingSpreadMod = 0.0f;
...
float ASWGATA_Trace::GetCurrentSpread() const
{
...
void ASWGATA_Trace::SetStartLocation(const FGameplayAbilityTargetingLocationInfo& InStartLocation)
{
	StartLocation = InStartLocation;
}

void ASWGATA_Trace::SetShouldProduceTargetDataOnServer(bool bInShouldProduceTargetDataOnServer)
{
	ShouldProduceTargetDataOnServer = bInShouldProduceTargetDataOnServer;
}

void ASWGATA_Trace::SetDestroyOnConfirmation(bool bInDestroyOnConfirmation)
{
	bDestroyOnConfirmation = bInDestroyOnConfirmation;
}

void ASWGATA_Trace::StartTargeting(UGameplayAbility* Ability)
{
...
void ASWGATA_Trace::ConfirmTargetingAndContinue()
{
...
void ASWGATA_Trace::CancelTargeting()
{
...
void ASWGATA_Trace::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	DestroyReticleActors();
...
void ASWGATA_Trace::LineTraceWithFilter(TArray<FHitResult>& OutHitResults, const UWorld* World,
...
void ASWGATA_Trace::AimWithPlayerController(const AActor* InSourceActor, FCollisionQueryParams Params,
...
bool ASWGATA_Trace::ClipCameraRayToAbilityRange(FVector CameraLocation, FVector CameraDirection, FVector AbilityCenter,
...
void ASWGATA_Trace::StopTargeting()
{
	SetActorTickEnabled(false);
...
FGameplayAbilityTargetDataHandle ASWGATA_Trace::MakeTargetData(const TArray<FHitResult>& HitResults) const
{
...
TArray<FHitResult> ASWGATA_Trace::PerformTrace(AActor* InSourceActor)
{
...
AGameplayAbilityWorldReticle* ASWGATA_Trace::SpawnReticleActor(FVector Location, FRotator Rotation)
{
	if (ReticleClass)
	{
...
void ASWGATA_Trace::DestroyReticleActors()
{
...
// Called every frame
void ASWGATA_Trace::Tick(float DeltaTime)
{
