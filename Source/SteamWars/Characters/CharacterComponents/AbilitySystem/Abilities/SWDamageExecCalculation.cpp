#include "SWDamageExecCalculation.h"
...
struct GDDamageStatics
{
	...
	GDDamageStatics()
	{
		...
		DEFINE_ATTRIBUTE_CAPTUREDEF(USWAttributeSet, Damage, Source, true);

		// Capture the Target's Armor. Don't snapshot.
		//DEFINE_ATTRIBUTE_CAPTUREDEF(USWAttributeSet, Armor, Target, false);
	}
};
...
USWDamageExecCalculation::USWDamageExecCalculation()
{
	RelevantAttributesToCapture.Add(DamageStatics().DamageDef);
	//RelevantAttributesToCapture.Add(DamageStatics().ArmorDef);
}

void USWDamageExecCalculation::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams,
	FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	USWAbilitySystemComponent* TargetASC = Cast<USWAbilitySystemComponent>(TargetAbilitySystemComponent);
	if (TargetASC)
	{
